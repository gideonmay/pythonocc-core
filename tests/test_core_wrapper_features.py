...
class TestWrapperFeatures(unittest.TestCase):
    ...
    def test_dump_to_string(self):
        ...
        # write to file
        output = open("./tests/test_io/box_shape_generated.brep", "wb")
        output.write(shp_dump)
        output.close()
        assert os.path.isfile("./tests/test_io/box_shape_generated.brep")

    def test_pickle_from_file(self):
        '''
        Checks if the pickle python module works for TopoDS_Shapes
        '''
        shp_dump = open("./tests/test_io/box_shape.brep", "rb")
